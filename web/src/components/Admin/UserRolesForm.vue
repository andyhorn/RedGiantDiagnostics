  ...
  <div class="container my-3">
    ...
    >
      ...
      >
        ...
        >
          Administrator (Enabling this will give this user full Administrative
          rights)
        </b-form-checkbox>
        <b-form-checkbox
          switch
      ...
      <b-button type="submit" variant="primary" :disabled="!submitEnabled"
        >Save</b-button
      >
    </b-form>
  </div>
...
export default {
  ...
  watch: {
    isUser: function() {
      if (!this.isUser) this.isAdmin = false;
    }
  },
  mounted() {
    this.fetchUserData();
  ...
  computed: {
    submitEnabled() {
      return (
        this.isUser != this.currentUserSetting ||
        this.isAdmin != this.currentAdminSetting
      );
    }
  },
  methods: {
    ...
    async onSubmit() {
      ...
      if (success) {
        ...
        toastService.errorToast(
          "Error",
          "There was an error saving the user roles."
        );
      }
    }
  }
